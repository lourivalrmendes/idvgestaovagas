...
export default function JobDetail() {
  ...
  useEffect(() => {
    if (vaga) {
      ...
      store.getHistoricoByVaga(vaga.dbId).then((h) => {
        setHistorico(h);
  ...
  const diasSLA = vaga.data_validacao_rh
    ? Math.floor((Date.now() - new Date(vaga.data_validacao_rh).getTime()) / 86400000)
    : null;
  ...
  const handleStatusChange = async () => {
    if (!newStatus) return;
    if (newStatus === "VAGA_REPROVADA" && !statusObs.trim()) {
      toast.error("Informe o motivo");
      return;
    }
    if (newStatus === "VAGA_APROVADA" && !statusDate) {
      toast.error("Informe data de início");
      return;
    }
    if (newStatus === "VAGA_APROVADA" && statusDate)
      await store.updateVaga(vaga.dbId, { data_prevista_inicio: statusDate });
  ...
  const handleValidar = async () => {
    if (!validarRecrutador) {
      toast.error("Selecione um recrutador");
      return;
    }
    await store.updateVaga(vaga.dbId, {
      recrutador_user_id: validarRecrutador,
      data_validacao_rh: new Date().toISOString().split("T")[0],
    });
  ...
  const handleAssociar = async () => {
    ...
    await store.addEnvio({
      vaga_id: vaga.dbId,
      candidato_id: assocCandidato,
      data_envio: new Date().toISOString().split("T")[0],
      status_candidato_na_vaga: "EM_ENTREVISTA",
      observacoes: assocObs,
  ...
  return (
    <div className="page-container animate-fade-in">
      <Button variant="ghost" onClick={() => navigate("/vagas")} className="mb-2">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-muted-foreground">
            {vaga.nome_cliente} · {vaga.unidade_negocio} · {vaga.categoria}
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {canValidate && vaga.status === "EM_VALIDACAO_RH" && !vaga.reprovada_rh_motivo && (
            <>
              ...
              <Button variant="destructive" onClick={() => setReprovarDialog(true)}>
                Reprovar Vaga
              </Button>
            </>
          )}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div>
                  ...
                  {diasSLA !== null ? (
                    <Badge variant={diasSLA > 15 ? "destructive" : diasSLA > 5 ? "default" : "secondary"}>
                      {diasSLA} dias
                    </Badge>
                  ) : (
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  {field("Faixa Salarial Candidato", vaga.faixa_salarial)}
                  {field(
                    "Tipo",
                    [
                  ...
                  {field(
                    "Modalidade",
                    [
                      vaga.modalidade_contratacao.presencial && "Presencial",
                      vaga.modalidade_contratacao.hibrido && "Híbrido",
                      vaga.modalidade_contratacao.remoto && "Remoto",
                    ]
                      .filter(Boolean)
                      .join(", "),
                  )}
        ...
        <TabsContent value="candidatos" className="mt-4 space-y-4">
          ...
          {envios.length > 0 ? (
                ...
                {envios.map((e) => {
                  ...
                  return (
                    <TableRow key={e.id}>
                      ...
                      <TableCell>
                        <CandidatoStatusBadge status={e.status_candidato_na_vaga as CandidatoStatusVaga} />
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {cand && (
                            <Button variant="ghost" size="sm" onClick={() => navigate(`/candidatos/${cand.id}`)}>
                              Ver
                            </Button>
                          )}
                          <Select
                            onValueChange={(val) => {
                              store.updateEnvioStatus(e.id, val);
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="EM_ENTREVISTA">Entrevista com R&S</SelectItem>
                              <SelectItem value="EM_ENTREVISTA_TECNICA">Entrevista técnica</SelectItem>
                              <SelectItem value="ENVIADO_COMERCIAL_CLIENTE">CV Enviado ao comercial/cliente</SelectItem>
                              <SelectItem value="EM_ENTREVISTA_CLIENTE">Entrevista com cliente</SelectItem>
                              <SelectItem value="REPROVADO">Reprovado</SelectItem>
                              <SelectItem value="APROVADO">Aprovado</SelectItem>
                              <SelectItem value="DECLINOU">Declinou</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                      </TableCell>
            ...
            <div className="text-center py-12 text-muted-foreground border border-dashed rounded-lg">
              Nenhum candidato enviado para esta vaga.
            </div>
          )}
        </TabsContent>
            ...
            <CardContent className="pt-6">
              {loadingHistorico ? (
                ...
                <div className="space-y-4">
                      ...
                      <div key={h.id} className="flex gap-4 items-start">
                        ...
                        <div>
                          <p className="text-sm font-medium">
                            {h.status_anterior
                              ? `${STATUS_LABELS[h.status_anterior as VagaStatus] || h.status_anterior} → `
                              : ""}
                            {STATUS_LABELS[h.status_novo as VagaStatus] || h.status_novo}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            {user?.nome || "Sistema"} · {new Date(h.criado_em).toLocaleDateString("pt-BR")}
                          </p>
                  ...
                  {historico.length === 0 && (
                    <p className="text-muted-foreground text-center py-4">Sem histórico registrado.</p>
                  )}
                </div>
              )}
            </CardContent>
      ...
      <Dialog open={statusDialog} onOpenChange={setStatusDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Atualizar Status — {vaga.id}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={validarDialog} onOpenChange={setValidarDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Validar e Atribuir Recrutador</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setValidarDialog(false)}>
              Cancelar
            </Button>
      ...
      <Dialog open={reprovarDialog} onOpenChange={setReprovarDialog}>
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={() => setReprovarDialog(false)}>
              Cancelar
            </Button>
      ...
      <Dialog open={assocDialog} onOpenChange={setAssocDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <Textarea
              value={assocObs}
              onChange={(e) => setAssocObs(e.target.value)}
              placeholder="Observações"
              rows={2}
            />
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setAssocDialog(false)}>
              Cancelar
            </Button>
